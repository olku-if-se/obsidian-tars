...
class GenerationStatsModal extends Modal {
  ...
  onOpen() {
    ...
    statsContainer.createEl('p', {
      text: `${t('Vendor')}: ${this.stats.vendor}`,
    })
    statsContainer.createEl('p', {
      text: `${t('Characters')}: ${this.stats.characters}`,
    })
    statsContainer.createEl('p', {
      text: `${t('Duration')}: ${this.stats.duration}`,
    })
    statsContainer.createEl('p', {
      text: `${t('Start Time')}: ${this.stats.startTime.toLocaleTimeString()}`,
    })
    statsContainer.createEl('p', {
      text: `${t('End Time')}: ${this.stats.endTime.toLocaleTimeString()}`,
    })
  }
...
class ErrorDetailModal extends Modal {
  ...
  onOpen() {
    ...
    errorContainer.createEl('p', {
      text: `${t('Error Type')}: ${this.error.name || t('Unknown Error')}`,
    })
    errorContainer.createEl('p', {
      text: `${t('Error Message')}: ${this.error.message}`,
    })
    errorContainer.createEl('p', {
      text: `${t('Occurrence Time')}: ${this.error.timestamp.toLocaleString()}`,
    })
    ...
    if (Platform.isDesktopApp) {
      const buttonContainer = contentEl.createDiv({
        cls: 'error-modal-buttons',
      })
      const copyBtn = buttonContainer.createEl('button', {
        text: t('Copy Error Info'),
      })
      copyBtn.onclick = () => {
