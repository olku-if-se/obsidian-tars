...
export class TagEditorSuggest extends EditorSuggest<TagEntry> {
  ...
  onTrigger(
    ...
    return {
      start: {
        line: cursor.line,
        ch: index > 0 && text[index - 1] === '#' ? index - 1 : index,
      },
  ...
  renderSuggestion(element: TagEntry, el: HTMLElement) {
    ...
    switch (element.role) {
      ...
      case 'user': {
        el.createSpan({
          text: `${element.replacement}  ${this.settings.roleEmojis.user}  `,
        })
        break
      }
      case 'system': {
        el.createSpan({
          text: `${element.replacement}  ${this.settings.roleEmojis.system}  `,
        })
        break
      }
      case 'newChat': {
        el.createSpan({
          text: `${element.replacement}  ${this.settings.roleEmojis.newChat}  `,
        })
        break
      }
      default: {
