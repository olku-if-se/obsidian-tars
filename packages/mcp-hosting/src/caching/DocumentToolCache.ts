...
export class DocumentToolCache {
	...
	getAllResults(editor: { getValue(): string }): CachedToolResult[] {
	...
	private parseCalloutBlock(lines: string[], startLine: number): ParsedCalloutBlock | null {
		...
		try {
			// Simple YAML parsing for tool parameters
			// In a real implementation, you'd use a proper YAML parser
			parsedYaml = this.parseSimpleYAML(toolYamlSource)
		} catch {
			parsedYaml = null
		}
	...
	private parseSimpleYAML(yamlText: string): Record<string, unknown> {
		...
		for (const line of lines) {
			...
			// Simple value parsing (in production, use proper YAML parser)
