...
export class ToolExecutor {
	...
	private readonly statusReporter?: IStatusReporter
	private readonly notificationHandler?: INotificationHandler
	...
	private async executeToolInternal(request: ToolExecutionRequest): Promise<ToolExecutionResult> {
		...
		try {
			...
			if (signal?.aborted) {
				...
				// Report error to status reporter with sanitized context
				this.statusReporter?.reportError('tool', `Tool execution failed: ${request.toolName}`, error as Error, {
