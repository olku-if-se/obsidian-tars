 ...
 * - Uses abstract interfaces for host integration (logging, status reporting)
 */
...
const _logger = {
	debug: (message: string, context?: Record<string, unknown>) =>
		console.debug(`[MCP:Manager] ${message}`, context ?? ''),
	info: (message: string, context?: Record<string, unknown>) => console.info(`[MCP:Manager] ${message}`, context ?? ''),
	warn: (message: string, context?: Record<string, unknown>) => console.warn(`[MCP:Manager] ${message}`, context ?? ''),
	error: (message: string, error?: Error, context?: Record<string, unknown>) =>
		console.error(`[MCP:Manager] ${message}`, error ?? '', context ?? '')
}
...
export class MCPServerManager extends EventEmitter<MCPServerManagerEvents> implements ToolServerAccessor {
	...
	private statusReporter?: IStatusReporter
	...
	constructor(options?: { logger?: ILogger; statusReporter?: IStatusReporter }) {
		super()
	...
	async initialize(
		...
		if (process.env.NODE_ENV !== 'test' && process.env.VITEST !== 'true') {
			for (const config of customConfigs) {
				this.logger.warn('unsupported server configuration format detected', {
					serverId: config.id,
					name: config.name
				})
			}
			...
			if (Object.keys(mcpUseConfig.mcpServers).length > 0) {
				...
				const sessionPromises = mcpUseConfigs.map(async (config) => {
					try {
						...
						this.logger.error(`Failed to create session for ${config.id}`, error as Error)

						// Report error to status reporter
						this.statusReporter?.reportError('mcp', `Failed to start MCP server: ${config.name}`, error as Error, {
							serverId: config.id,
		...
		try {
			...
			await withRetry(
				...
				(attempt: number, error: Error, nextRetryIn: number) => {
					...
					if (process.env.NODE_ENV !== 'test' && process.env.VITEST !== 'true') {
						this.logger.warn('retrying server start', {
							serverId,
							attempt,
	...
	async stopServer(serverId: string): Promise<void> {
		...
		try {
			...
			this.logger.error(`Error stopping server ${serverId}`, error as Error)

			// Report error to status reporter
			this.statusReporter?.reportError('mcp', `Failed to stop MCP server: ${config.name}`, error as Error, {
				serverId: config.id,
				serverName: config.name
			})
	...
	async shutdown(): Promise<void> {
		if (this.mcpClient) {
			try {
				...
				this.logger.error('Error closing MCP sessions', error as Error)
			}
			this.mcpClient = null
		}
	...
	private checkAndAutoDisable(config: MCPServerConfig): void {
		if (config.failureCount >= this.failureThreshold && config.enabled && !config.autoDisabled) {
			...
			if (process.env.NODE_ENV !== 'test' && process.env.VITEST !== 'true') {
				this.logger.warn('server auto-disabled after repeated failures', {
					serverId: config.id,
					failureCount: config.failureCount
				})
			}
