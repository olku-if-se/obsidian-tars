...
export async function safeAsync<T>(operation: () => Promise<T>, fallback: T, errorMessage: string): Promise<T> {
	try {
		...
		console.warn(`[MCP:Utils] ${formatErrorWithContext(errorMessage, error)}`)
		return fallback
	}
}
...
export function logError(context: string, error: unknown): void {
	console.error(`[MCP:Utils] ${formatErrorWithContext(context, error)}`)
}
...
export function logWarning(context: string, error: unknown): void {
	console.warn(`[MCP:Utils] ${formatErrorWithContext(context, error)}`)
}
