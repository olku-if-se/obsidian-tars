...
// APP_FOLDER will be provided by the consuming package
import { createLogger } from '@tars/logger'
import type { CreatePlainText, SendRequest } from './base'
...
export const withStreamLogging = (originalFunc: SendRequest, createPlainText: CreatePlainText, appFolder: string = 'Tars'): SendRequest => {
	return async function* (messages, controller, resolveEmbedAsBinary, saveAttachment) {
		...
		try {
			...
			const filePath = normalizePath(`${appFolder}/${formatDate(new Date())}-${brief}.json`)
