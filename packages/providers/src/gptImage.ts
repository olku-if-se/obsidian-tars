...
import type { BaseOptions, Message, ResolveEmbedAsBinary, SaveAttachment, SendRequest, Vendor } from './base'
...
const sendRequestFunc = (settings: GptImageOptions): SendRequest =>
	async function* (
		...
		if (messages.length > 1) {
			new Notice('Only the last user message is used for image generation. Other messages are ignored.')
		}
		...
		new Notice('This is a non-streaming request, please wait...', 5 * 1000)
		let response = null
		if (lastMsg.embeds && lastMsg.embeds.length > 0) {
			...
			if (!supportedMimeTypes.includes(mimeType)) {
				throw new Error('Only PNG, JPEG, and WebP images are supported for editing.')
			}
		...
		if (!response.data || response.data.length === 0) {
			throw new Error('Failed to generate image. no data received from API')
		}
		yield ' \n'
		const now = new Date()
		const formatTime =
