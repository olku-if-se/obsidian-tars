...
describe('ClaudeDIProvider', () => {
	...
	it('should be instantiable with DI services', () => {
		expect(provider).toBeInstanceOf(ClaudeDIProvider)
	})
	...
	it('should have models from the original Claude vendor', () => {
	...
	it('should format error messages correctly', () => {
		...
		expect(formattedError).toContain('Claude Error in test context: Test error')
	...
	it('should get capability emoji correctly', () => {
		...
		expect(provider.getCapabilityEmoji('PDF Vision')).toBe('📄')
	...
	it('should handle createSendRequest with DI integration', async () => {
		...
		// Note: We can't easily test the actual stream without mocking the Claude API
	...
	it('should handle missing DI services gracefully', () => {
		...
		const providerWithNullServices = new ClaudeDIProvider(
			mockLoggingService,
			mockNotificationService,
			mockSettingsService,
			mockDocumentService
