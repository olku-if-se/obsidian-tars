...
import { OpenAIDIProvider } from '../openai-di'
import {
	ILoggingService,
...
describe('OpenAIDIProvider', () => {
	let provider: OpenAIDIProvider
	...
	it('should be instantiable with DI services', () => {
		expect(provider).toBeInstanceOf(OpenAIDIProvider)
	})

	it('should have correct provider metadata', () => {
		expect(provider.name).toBe('OpenAI')
		expect(provider.websiteToObtainKey).toBe('https://platform.openai.com/api-keys')
		expect(provider.capabilities).toContain('Text Generation')
		expect(provider.capabilities).toContain('Image Vision')
		expect(provider.capabilities).toContain('Image Generation')
		expect(provider.capabilities).toContain('Tool Calling')
		expect(provider.capabilities).toContain('Reasoning')
	})

	it('should have models from the original OpenAI vendor', () => {
	...
	it('should format error messages correctly', () => {
		...
		expect(formattedError).toContain('OpenAI Error in test context: Test error')
	...
	it('should get capability emoji correctly', () => {
		...
		expect(provider.getCapabilityEmoji('Image Generation')).toBe('🎨')
	...
	it('should handle createSendRequest with DI integration', async () => {
		...
		// Note: We can't easily test the actual stream without mocking the OpenAI API
	...
	it('should handle missing DI services gracefully', () => {
		...
		const providerWithNullServices = new OpenAIDIProvider(
			mockLoggingService,
			mockNotificationService,
			mockSettingsService,
			mockDocumentService
	...
	it('should have correct OpenAI-specific models', () => {
