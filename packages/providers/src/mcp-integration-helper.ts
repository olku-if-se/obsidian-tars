...
export class MCPIntegrationHelper {
	...
	hasToolCalling(): boolean {
		return !!(
			this.mcpIntegration?.toolCallingCoordinator &&
			this.mcpIntegration?.providerAdapter
		);
	}
	...
	async *generateWithTools(
		...
		try {
			...
			logger.warn(
				"Tool calling failed, falling back to regular generation",
				error,
			);
			throw error;
		}
	...
	async *generateWithFallback(
		...
		if (this.hasToolCalling()) {
			try {
				...
				logger.warn(
					"Tool calling path failed, trying tool injection fallback",
					{
						error: error instanceof Error ? error.message : String(error),
						providerName,
					},
			...
			logger.info(
				`Tool calling not available for ${providerName}, using tool injection`,
			);
		}
		...
		throw new Error(
			`Tool injection fallback not implemented for ${providerName}`,
		);
	}
}
 ...
 */
export function createMCPIntegrationHelper(
	settings: BaseOptions,
): MCPIntegrationHelper | null {
	try {
