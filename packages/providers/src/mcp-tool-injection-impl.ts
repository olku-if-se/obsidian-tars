...
export class ConcreteMCPToolInjector implements McpToolInjector {
	...
	private validateToolSchema(tool: any): boolean {
		...
		if (schema.type !== "object") {
			console.warn(
				`Tool ${tool.name} has invalid schema: missing or invalid type property`,
			);
			return false;
		}
		...
		if (!schema.properties || typeof schema.properties !== "object") {
			console.warn(
				`Tool ${tool.name} has invalid schema: missing or invalid properties`,
			);
			return false;
		}
		...
		for (const [propName, propSchema] of Object.entries(schema.properties)) {
			if (
				...
				console.warn(
					`Tool ${tool.name} has invalid property schema for ${propName}: missing type`,
				);
				return false;
			}
			...
			if (!validTypes.includes(propSchema.type as string)) {
				console.warn(
					`Tool ${tool.name} has invalid property type for ${propName}: ${propSchema.type}`,
				);
				return false;
			}
			...
			if (!("description" in propSchema) || !propSchema.description) {
				console.warn(
					`Tool ${tool.name} property ${propName} is missing description - rejecting tool`,
				);
				return false;
			}
		}
		...
		if (schema.required && !Array.isArray(schema.required)) {
			console.warn(
				`Tool ${tool.name} has invalid required field: must be an array`,
			);
			return false;
		}
