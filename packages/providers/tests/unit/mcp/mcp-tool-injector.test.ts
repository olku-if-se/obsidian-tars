...
describe('MCPToolInjector', () => {
	...
	describe('Error handling', () => {
		it('should handle tool discovery failures gracefully', async () => {
			...
			vi.mocked(injector.injectTools).mockRejectedValue(new Error('Failed to discover MCP tools'))

			await expect(injector.injectTools(parameters, 'OpenAI')).rejects.toThrow('Failed to discover MCP tools')
		})
		...
		it('should handle schema validation errors', async () => {
			...
			await expect(injector.injectTools({ model: 'test' }, 'OpenAI')).rejects.toThrow('Invalid tool schema')
		})
	})
