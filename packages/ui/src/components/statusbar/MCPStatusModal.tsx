...
import { formatSessionStatus, formatRetryInfo, getServerStatusIcon, getServerStatusText, type MCPServerInfo } from './utilities'
import { ParagraphList, InfoSectionList, TabList } from './atoms'
...
export const MCPStatusModal: React.FC<MCPStatusModalProps> = ({
	...
	const [activeTab, setActiveTab] = useState<'mcp' | 'errors'>(
		currentError ? 'errors' : 'mcp'
	)
	...
	const ServerStatusItem = useCallback(({ server }: { server: MCPServerInfo }) => {
		...
		return (
			<div key={`${server.id}-${server.name}`} className={styles.serverItem}>
				<InfoSectionList
					sections={serverInfoSections}
					containerClassName={styles.serverInfo}
				/>
			</div>
		)
	}, [])
			...
			...(mcpStatus.currentDocumentSessions !== undefined &&
				...
				mcpStatus.sessionLimit > 0 ? (() => {
					const sessionStatus = formatSessionStatus(
						mcpStatus.currentDocumentSessions,
						mcpStatus.sessionLimit,
						styles
					)
					return [{
		...
		<Modal isOpen={true} onClose={onClose} title="MCP Server Status" size="lg">
			<div className={styles.mcpStatusModal}>
				<TabList
					tabs={tabs}
					activeTab={activeTab}
					onTabChange={(tabId: string) => tabId === 'mcp' || tabId === 'errors' ? setActiveTab(tabId as 'mcp' | 'errors') : undefined}
