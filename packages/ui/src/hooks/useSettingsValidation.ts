...
import { URLValidator, TagValidator, JSONValidator, VendorValidator, SettingsValidator } from '../utils/settingsValidation'

/**
 * Hook for validating URLs with real-time feedback
 */
export function useURLValidation(initialValue: string = '', options?: {
	...
	const [validation, setValidation] = useState<ValidationResult>(
		URLValidator.isValid(initialValue, options)
	)
 ...
 */
export function useTagValidation(initialTags: string[] = [], options?: {
	allowEmpty?: boolean
	...
	const [validation, setValidation] = useState<TagValidationResult>(
		TagValidator.validateTags(initialTags, options)
	)

	const validate = useCallback((newTags: string[]) => {
	...
	const updateTag = useCallback((index: number, newTag: string) => {
 ...
 */
export function useJSONValidation(initialValue: string = '', options?: {
	allowEmpty?: boolean
	schema?: any
	maxSize?: number
}) {
	const [value, setValue] = useState(initialValue)
	const [validation, setValidation] = useState<ValidationResult>(
		JSONValidator.isValid(initialValue, options)
	)
...
export function useVendorValidation(vendor: string, initialConfig: any = {}) {
	...
	const validate = useCallback((newConfig: any) => {
	...
	const handleChange = useCallback((newConfig: any) => {
		setConfig(newConfig)
...
export function useProviderValidation(initialProvider: any = {}) {
	...
	const [validation, setValidation] = useState<ValidationResult>(
		SettingsValidator.validateProvider(initialProvider)
	)
	...
	const handleChange = useCallback((newProvider: any) => {
