...
if [[ -z "$KEY_NAME" ]] || [[ -z "$KEY_VALUE" ]]; then
  ...
  echo ""
  echo "Examples:"
  echo "  mise run secrets-rotate OPENAI_API_KEY sk-proj-new-key-here"
  echo "  mise run secrets-rotate ANTHROPIC_API_KEY sk-ant-new-key-here"
  echo "  mise run secrets-rotate GROK_API_KEY xai-new-key-here"
  echo ""
  echo "Available keys:"
